...
class Individuo:
    ...
    def mejorar_2opt(self, k=100):
       ...
       num_llamadas_objetivo = n_llamadas_ojetivo + num_llamadas_objetivo
       for _ in range(k):
...
class Poblacion:
    def __init__(self, tamaño, n_ciudades, limit_llamadas_funcion_objetivo, limite_generaciones,  p_elite, p_diversos, tamaño_torneo,
        ...
        self.limit_llamadas_funcion_objetivo = limit_llamadas_funcion_objetivo
    ...
    def inicializar(self, tamaño, n):
        # Crear 1000 individuos aleatorios
        individuos = [Individuo(np.random.permutation(n).tolist()) for _ in range(1000)]
        
        # Ordenar los individuos por su fitness (de menor a mayor)
        individuos.sort(key=lambda ind: ind.fitness)
        
        # Seleccionar los mejores `tamaño` individuos
        poblacion = individuos[:tamaño]
    ...
    def evolucionar(self):
        ...
        i = 0
        while num_llamadas_objetivo < self.limit_llamadas_funcion_objetivo:
            i = i + 1
            ...
            # Cruzar la población 
            hijos = self.recombinacion(padres) 
            self.individuos = hijos

            # Mutar
            self.mutacion(self.prob_mutacion, self.nivel_mutacion)

            # Agregamos los mejores de la poblacion anterior
...
def memetico(matriz, num_generaciones=100, tam_pobla=1000, limit_llamadas_funcion_objetivo = None, p_elite = 0.05, p_diversos = 0.05, tamaño_torneo = 0.1,
    ...
    # poblacion = Poblacion(tam_pobla, n)

    if (limit_llamadas_funcion_objetivo is None):
        limite_generaciones = num_generaciones
    else:
        # Calcular cantidad de ciclos
        limite_generaciones = int(limit_llamadas_funcion_objetivo / tam_pobla) + 1

    poblacion = Poblacion(
        tamaño=tam_pobla,
        n_ciudades=n,
        limite_generaciones=limite_generaciones,
        limit_llamadas_funcion_objetivo=limit_llamadas_funcion_objetivo,
        p_elite=p_elite,
    ...
    #poblacion.inicializar_goloso(matriz_distancias)
    #poblacion.mutacion(1, 0.15)
    poblacion.evolucionar()

    #poblacion.imprimir_poblacion()
    # Inicializar la matriz de fitness
   
    return poblacion.mejor_individuo()  # Devolver el mejor individuo
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ruta", type=str, default='Instancias/matrix_cir150.txt', help="Ruta al archivo de matriz de distancias")
    parser.add_argument("--n_generaciones", type=int, default=150, help="Número de generaciones")
    parser.add_argument("--tam_pobla", type=int, default=40, help="Tamaño de la población")
    parser.add_argument("--limit_llamadas_funcion_objetivo", type=int, default=3000000, help="Número de llamadas a la función objetivo límite")
    parser.add_argument("--p_elite", type=float, default=0.1, help="Proporción de individuos élite")
    parser.add_argument("--p_diversos", type=float, default=0.25, help="Proporción de individuos diversos")
    ...
    parser.add_argument("--profundidad_ls", type=int, default=300, help="Profundidad de búsqueda local")
    ...
    error_relativo = abs(mejor_solucion.fitness - 95345)/ 95345 * 100
    #print(f"{error_relativo:.4f}")


    # Imprimir el tiempo de ejecución
    #print(mejor_solucion.fitness , end_time-start_time)
    mejor_solucion.evaluar_fitness_numpy()
...
# python3 geneticoLS.py --ruta Instancias/matrix-wi29.txt --optimo 27603
# python3 geneticoLS.py --ruta Instancias/matrix-dj38.txt --optimo 6656
# python3 geneticoLS.py --ruta Instancias/matrix-qa194.txt --optimo 9352
# python3 geneticoLS.py --ruta Instancias/matrix-uy734.txt --optimo 79114
# python3 geneticoLS.py --ruta Instancias/matrix-zi929.txt --optimo 95345
# python3 geneticoLS.py --ruta Instancias/matrix-lu980.txt --optimo 11340
# python3 geneticoLS.py --ruta Instancias/matrix-rw1621.txt --optimo 26051
